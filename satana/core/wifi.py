...
def set_monitor_mode(interface: str) -> tuple[int, str, str]:
    ...
    out_iw = out_mon = err_iw = err_mon = stderr_down = ""
    ip_link_down = 0

    if os.path.exists(con_mode_path):
        # ICNSS2 (Qualcomm) special handling
        ip_link_down, _, stderr_down = run_command(["ip", "link", "set", interface, "down"], timeout=10)
        run_command(["bash", "-c", f"echo 4 > {con_mode_path}"], timeout=5)
    ...
    return 1, json.dumps(payload, indent=2, ensure_ascii=False) + "\n", combined_stderr
...
def set_managed_mode(interface: str) -> tuple[int, str, str]:
    ...
    if os.path.exists(con_mode_path):
        ...
        run_command(["bash", "-c", f"echo 0 > {con_mode_path}"], timeout=5)
        run_command(["ip", "link", "set", interface, "up"], timeout=10)
    else:
        run_command(["ip", "link", "set", interface, "down"], timeout=10)
        _, out_iw, err_iw = run_command(["iw", interface, "set", "type", "managed"], timeout=15)
        run_command(["ip", "link", "set", interface, "up"], timeout=10)
